...
/**
 * This function executes a While command. At each iteration it checks a condition
 * and if the condition is true, it executes all the commands in the scope.
 * A command can be a command from the CommandsMap or setting a var from Symbol Table.
 ...
 * till the next command after the While scope.
 *
 ...
 */
int LoopCommand::execute(vector<string> &strArr, int index) {
    // Indication for entering to the While scope
    int flag = 0;
    int originIndex = index;
    // Update the current index after this While condition
    ...
    while (this->conditionIsTrue(leftExp, sign, rightExp)) {
        // Indicate entering into the scope
        flag = 1;
        curIndex = originIndex + 5;
